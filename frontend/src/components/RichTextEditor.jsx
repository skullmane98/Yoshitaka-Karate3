...
export default function RichTextEditor({ value, onChange, placeholder, minHeight = 160, testid }) {
  ...
  const Btn = ({ active, onClick, children, label }) => (
    <button
      ...
      className={`px-2 py-1 text-xs border border-[#DCD9CF] transition-colors ${
        active ? "bg-[#0F0F0F] text-white border-[#0F0F0F]" : "bg-white text-[#0F0F0F] hover:border-[#0F0F0F]"
      }`}
    >
      {children}
    </button>
  ...
  return (
    <div className="border border-[#DCD9CF] bg-white" data-testid="rte-root">
      <div className="flex flex-wrap gap-1 p-2 border-b border-[#DCD9CF] bg-[#F1EEE5]">
        ...
        <span className="w-px h-5 bg-[#DCD9CF] mx-1 self-center" />
