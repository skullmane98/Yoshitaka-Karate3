        ...
        <form onSubmit={submit} className="space-y-5 border border-[#E0DCD0] bg-[#F7F5F0] p-8">
          <div>
            ...
            <input
              ...
              className="w-full border border-[#C1121F] bg-white px-4 py-3 font-mono-accent tracking-widest uppercase focus:outline-none"
            />
          </div>
          <div className="grid md:grid-cols-2 gap-5">
            <div>
              ...
              <input required value={form.name} onChange={set("name")} data-testid="register-name-input"
                className="w-full border border-[#E0DCD0] bg-white px-4 py-3 focus:outline-none focus:border-[#1A1A1A]" />
            </div>
            <div>
          ...
          {err && <div className="text-[#C1121F] text-sm" data-testid="register-error">{err}</div>}
          ...
          <div className="text-sm text-[#4A4A4A] text-center pt-2">
            Already enrolled? <Link to="/login" className="ink-underline text-[#1A1A1A]">Login</Link>
          </div>
        </form>
      </section>
    </PublicLayout>
